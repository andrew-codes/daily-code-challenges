...
- [x] Given a tab is active and the panel is focused, when tabbing to the next element, then focus is moved to the next focusable element in the active tab.
...
- [ ] Given a tab is active and the tabs are vertical, when using the left and right arrow keys, then they maintain their default behavior (scrolls up/down page).
